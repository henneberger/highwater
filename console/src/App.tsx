...
function ProcessesPage({ processes }: { processes: Process[] }) {
  return <><PageHeader eyebrow="Isolation" title="Processes" description="Per-key durable programs with explicit concurrency and backpressure." /><div className="resource-card-grid">{processes.map((process) => { const retrying = process.retrying ?? 0; const used = process.pending + process.running + retrying; return <Card className="resource-card process-card" key={process.process_id}><div className="resource-card-head"><span className="large-resource-icon green"><Layers3 size={20} /></span><Badge status={process.status}>{title(process.status)}</Badge></div><h3>{process.process_id}</h3><p className="resource-description">{process.workflow_type}</p><div className="queue-bar"><span style={{ width: `${Math.min(100, (used / Math.max(1, process.mailbox_capacity)) * 100)}%` }} /></div><div className="queue-label"><span>Mailbox utilization</span><b>{used} / {process.mailbox_capacity}</b></div><dl><div><dt>Pending</dt><dd>{process.pending}</dd></div><div><dt>Running</dt><dd>{process.running}</dd></div><div><dt>Retrying</dt><dd>{retrying}</dd></div><div><dt>Quarantined</dt><dd>{process.quarantined ?? 0}</dd></div><div><dt>Completed</dt><dd>{process.completed}</dd></div><div><dt>Failed</dt><dd>{process.failed}</dd></div></dl><div className="resource-footer"><Network size={14} />{process.max_concurrent_keys} normal · {process.retry_concurrency ?? 0} retry slots · {process.max_attempts ?? 0} attempts</div></Card>; })}</div>{processes.length === 0 && <Card><Empty icon={Layers3} title="No keyed processes" detail="Run the account balance example to deploy one." /></Card>}</>;
}
...
export default function App() {
  ...
  return <Shell page={page} setPage={setPage} onLogout={logout} online={!error} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen}>
    ...
    {!data ? <div className="page-loader"><RefreshCw className="spin" />Connecting to the demo cluster…</div> : <>
      ...
      {page === "runs" && !selectedRun && <RunsPage data={data} openRun={openRun} />}
