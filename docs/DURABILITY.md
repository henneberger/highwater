...
The durable source of truth is the partition journal, not local RocksDB and not the watermark. The control plane and keyed process lanes have independent sequences. A direct-ingress process key always hashes to the same lane, so its input, mailbox, lease, state, applied offset, terminal execution outcome, output changelog, and pending output effects share one atomic boundary. Stream-fed and direct-ingress execution are explicit, mutually exclusive process modes; the same process state cannot be mutated through both journal paths. RocksDB is updated after journal commitment with its own WAL disabled because it is a disposable materialized cache.
...
Process completion uses the same boundary. An executor leases several Process activations and returns several results, and the owning lane commits their state, input progress, terminal outcomes, output changelog, pending output effects, and task acknowledgements in one WAL record. If the commit fails, none of those completions are acknowledged. Each grant includes the partition owner epoch and a partition-wide activation sequence. Renewal and completion verify both against the current durable owner inside the lane transaction, so a delayed executor cannot commit after restart or reassignment.

Every admitted process event has a durable outcome keyed by process, key, and stable event ID. It begins `PENDING` and can reach exactly one terminal state: `COMMITTED` with deterministic output message IDs, or `FAILED` with the exhausted attempt count and failure. Direct ingress rejects missing event IDs and rejects reuse with different event contents.
...
| During a Python batch | No completion was committed. Lease expiry durably consumes an attempt and retries in the isolated lane; repeated worker loss eventually reaches the same quarantine boundary as returned handler failure. |
...
Process completion writes an immutable pending output marker in the same lane transaction as state and the terminal outcome. An idempotent handoff promotes that marker to the leased control-shard outbox under the same deterministic message ID. The marker is retained so any checkpoint cut can reconstruct a missing promotion; deleting it requires a future garbage-collection protocol tied to a published vector covering both shards. Delivery is at least once. A sink obtains effectively exactly-once application by enforcing uniqueness on the message ID or by participating in a checkpoint-aware transaction. Calling a non-idempotent external service directly from a batch handler is outside this guarantee.
