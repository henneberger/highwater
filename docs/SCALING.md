...
The programming model should remain one durable workflow or keyed process even as execution moves across machines. Distribution is an implementation detail below that contract: a key has one ordered mailbox, one active transition, bounded admission, and an atomic input/state/output commit.
...
Ready executions carry their input and prior state, and leases carry the complete activation batch. A worker crash therefore needs no reconstruction from transient Python memory. Success commits state, output, lease removal, active-key release, and the next mailbox dispatch together. Failure or lease expiry leaves the prior state intact and makes the same durable input runnable again.
...
5. Credit-based admission propagates capacity from worker pools through shard owners to producers. Overload delays or rejects admission before a source cursor is committed.
...
- expired worker leases rerun from committed prior state;
