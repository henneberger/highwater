...
`EventTimeGate.IMMEDIATE` dispatches as soon as capacity permits. `EventTimeGate.COMPLETE` dispatches only after the input completeness frontier passes the event timestamp, meaning complete under the configured source and late-data policy, rather than proof that no historical event can arrive. Sealing bounded input releases all remaining events. Source-managed connectors should continue emitting watermarks even while data admission is backpressured, because progress messages are intentionally not subject to mailbox capacity.

Stream inspection includes `finality`: its basis distinguishes inferred lateness cutoffs, explicit declarations covering every unsealed partition, and permanently sealed input. `complete_through` is a stream-wide declaration. With late policy `accept`, historical changes remain admissible even after the gate opens; arbitrary Python handlers do not automatically retract their effects.

A direct-ingress Process can call `handle.finalize(key)` after the key has no queued, running, or retrying events. The durable business declaration irreversibly rejects new events while preserving identical retries and repeated finalization. State inspection exposes the marker. This does not wait for output delivery or allow state or journal reclamation; checkpoint coverage remains the storage safety boundary.
