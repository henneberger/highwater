...
Mailbox admission and source cursor advancement happen in the same transaction. `pending + running == capacity` returns retryable HTTP 429; `StreamWriter` pauses and retries without advancing its durable offset. Workflow leases prevent concurrent handler activations, and the active-key permit is released in the same commit that stores the result state and schedules the next item. Failed, cancelled, terminated, and timed-out handlers leave prior state unchanged, release the key, and increment the failure count.
...
This is not yet a distributed Flink replacement. It now has the research-grade correctness mechanisms for native changelog edges, fenced multi-owner checkpoint barriers, and retractable interval-join arrangements. The main remaining limits are remote shard executors that execute the barrier protocol, credit-based backpressure across edges, unaligned/incremental checkpoints, rescaling transfer, and a general multi-input operator scheduler. The current coordinator exposes the remote protocol but the shipped binary still owns local execution.
