...
class Registry:
    ...
    def register_workflow(self, cls: type) -> type:
        name = getattr(cls, "__highwater_workflow__", None)
        if not name:
            raise TypeError(f"{cls.__name__} is missing @workflow.defn")
        runs = [member for _, member in inspect.getmembers(cls) if getattr(member, "__highwater_kind__", None) == "run"]
        ...
        asynchronous_handlers = [
            ...
            if getattr(member, "__highwater_kind__", None) in {"signal", "update"}
    ...
    def discover(self, module: ModuleType) -> "Registry":
        for _, value in inspect.getmembers(module):
            if inspect.isclass(value) and getattr(value, "__highwater_workflow__", None):
                self.register_workflow(value)
            elif callable(value) and getattr(value, "__highwater_kind__", None) == "activity":
    ...
    def workflow_methods(self, workflow_type: str, kind: str) -> dict[str, str]:
        ...
        return {
            getattr(member, "__highwater_name__"): attr
            for attr, member in inspect.getmembers(cls)
            if getattr(member, "__highwater_kind__", None) == kind
        }

    def run_method(self, workflow_type: str) -> str:
        cls = self.workflows[workflow_type]
        return next(attr for attr, member in inspect.getmembers(cls) if getattr(member, "__highwater_kind__", None) == "run")

    def build_ids(self) -> list[str]:
        return sorted({
            build_id
            for workflow in self.workflows.values()
            if (build_id := getattr(workflow, "__highwater_build_id__", None))
        })
